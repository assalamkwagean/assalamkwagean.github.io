...
class Firestore {
  ...
  updateDocument(collection, id, data) {
    const url = `https://firestore.googleapis.com/v1/projects/${this.projectId}/databases/(default)/documents/${collection}/${id}?updateMask.fieldPaths=recap&updateMask.fieldPaths=updatedAt`;

    // Convert simple JSON to Firestore JSON format
...
/**
 * SYNC FUNCTION
 * Reads RECAP sheet and updates Firestore.
 */
function syncRecapToFirebase() {
  const firestore = new Firestore(FIREBASE_CONFIG);
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const recapSheet = ss.getSheetByName('RECAP');

  if (!recapSheet) {
    Logger.log("Sheet RECAP not found");
    return;
  }
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Map headers to values, skipping empty ones
    for (let j = 5; j < headers.length; j++) { // Assuming tagihan starts from column F (index 5)
      if (headers[j] && headers[j] !== "") {
        recapData[headers[j]] = row[j] || 0;
      }
    }
    ...
    firestore.updateDocument('tagihan', String(nis), payload);
  }
}
...
function setupTrigger() {
  ScriptApp.newTrigger('syncRecapToFirebase')
    .timeBased()
    .everyHours(1)
    .create();
}
